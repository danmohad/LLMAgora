...
def fixed_model_family_summary(
    ...
    *,
    ...
    """F1a: lollipop — x = scenario, y = mean gap, colour = family."""
    agg = summarize.cross_scenario(df, model=model, metric=metric)
    col = f"mean_{metric}"
    ...
    for fi, fam in enumerate(families):
        ...
        ax.plot(x, vals, "o", color=color, markersize=8, label=fam)

    ax.set_xticks(np.arange(len(scenarios)) + bar_width * (n_fam - 1) / 2)
    ax.set_xticklabels([scenario_title(s) for s in scenarios], rotation=20, ha="right", fontsize=9)
...
def fixed_model_family_heatmap(
    ...
    *,
    ...
    """F1b: heatmap — rows = scenario, columns = family."""
    agg = summarize.cross_scenario(df, model=model, metric=metric)
    col = f"mean_{metric}"
    ...
    fig, ax = plt.subplots(figsize=(7, max(3, len(piv) * 0.8)))
...
def fixed_model_turn_dynamics(
    ...
    *,
    ...
    """F2: x = turn, y = mean gap, facet by family, colour = scenario."""
    ...
    for ax, fam in zip(axes, families):
        for scen in scenarios:
            sf = sub[(sub["scenario"] == scen) & (sub["question_family"] == fam)]
            if sf.empty:
                continue
