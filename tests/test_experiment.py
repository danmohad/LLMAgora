...
def test_run_demo_exposes_snapshot_flags():
    ...
    for cell in notebook_payload["cells"]:
        if cell.get("cell_type") != "code":
            continue
        source.extend(cell.get("source", []))
    code = "".join(source)

    assert '"load_snapshot": False' in code
    assert '"load_dir": None' in code
    assert '"save_snapshot": True' in code
...
def test_build_experiment_config_and_helpers(tmp_path):
    ...
    with pytest.raises(ValueError):
        build_experiment_config({"scenario_id": "s1", "semantic_similarity_device": "bad"})
    with pytest.raises(ValueError):
